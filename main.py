...
<head>
    ...
    <style>
        ...
        .message { display: flex; margin-bottom: 20px; max-width: 80%; position: relative; }
        ...
        .delete-btn { background: #6c757d; color: white; border: none; border-radius: 4px; padding: 2px 6px; font-size: 12px; cursor: pointer; text-decoration: none; display: block; margin-top: 5px; text-align: center; }
            ...
            <div class="chat-history" id="chat-history">
                ...
                <div class="message {% if item.role == 'user' %}user-message{% else %}model-message{% endif %}">
                    <div class="message-bubble">
                        <p>{{ item.text }}</p>
                        {% if item.id %}
                        <a href="{{ url_for('delete_log', log_id=item.id) }}" class="delete-btn" onclick="return confirm('このログを削除しますか？');">削除</a>
                        {% endif %}
                    </div>
                </div>
                {% endfor %}
    ...
    <script>
        ...
        function loadSettings() {
            ...
            models.forEach(model => {
                ...
                option.value = model;
                option.text = model.replace('gemini-', 'Gemini ');
                if (model === (savedSettings.model_name || 'gemini-1.5-flash')) {
...
# --- ▼▼▼ Re-instated routes for consistency ▼▼▼ ---
@app.route('/clear_file')
def clear_file():
    # This route is no longer strictly needed as JS handles it,
    # but it's kept to prevent errors if old pages are cached.
...
def home():
    # Settings are now primarily loaded by JavaScript from localStorage.
    # Python just provides the initial HTML shell.
    display_history = []
    try:
        # Limit history display for faster initial load
        docs = db.collection('conversations').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(50).stream()
        history_docs = list(docs)
        history_docs.reverse() # Show oldest of the last 50 first
