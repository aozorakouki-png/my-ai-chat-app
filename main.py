...
def home():
    ...
    if saved_config:
        try:
            ...
            saved_config = None # Cookieが壊れていたら無視

    # 2. Cookieがなければデフォルト設定を使用
    if not saved_config:
        config = {
    ...
    if request.method == 'POST':
        ...
        if not api_key:
            ...
            try:
                ...
                # --- 会話履歴を読み込んでコンテキストを作成 ---
                chat_history = []
                docs = db.collection('conversations').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(4).stream()
                # Firestoreから取得した順（新しい→古い）と逆（古い→新しい）にリストへ追加
...
ステップ2：再デプロイする
GitHub上でmain.pyを上記の新しいコードに更新します。

変更を保存（Commit）すれば、自動的にCloud Runへのデプロイが始まります。

## 新しい機能の解説
設定の保存:

ページを開くと、まずブラウザのCookieに保存された設定を探しに行きます。

「送信」ボタンを押すと、その時の設定がCookieに上書き保存されます。

会話ログから学ぶ:

AIに質問を送る直前に、Firestoreから**直近4件（2往復分）**の会話履歴を読み込みます。

その履歴をAIに「これまでの流れはこうだよ」と教えた上で、新しい質問をします。

これにより、AIは「さっきの話の続きだね」と文脈を理解した応答ができるようになります。

これであなたのアプリは、よりパーソナルで、より文脈を理解する、非常にスマートなAIチャットに進化しました！
...
・更に自動的にGoogle検索を行いAI自身が知識として備える機能欲しいです。

・会話ログの任意の場所を消す機能がほしいです。



思考プロセスを表示
はい、承知いたしました。AI Studioのような高度な機能を自分のアプリに実装していくのは、素晴らしい挑戦ですね。

ご要望の2つの機能は、それぞれ異なる技術的なアプローチで実装できます。

Google検索と連携する機能: Geminiの**ツール（Function Calling）**という機能を使います。AIが「情報が足りない」と判断した際に、AI自身がGoogle検索ツールを呼び出すようにプログラムを組みます。

会話ログを削除する機能: 各会話ログにユニークなIDを割り当て、そのIDを指定してFirestoreデータベースからデータを削除する**専用のルート（URL）**をFlaskアプリケーションに追加します。

この2つの機能を両方とも実装した、新しいmain.pyのコードをご提案します。

## 実装手順
ステップ1：main.pyを更新する
GitHub上で、現在のmain.pyを以下の新しいコードに完全に置き換えてください。 このコードには、Google検索連携とログ削除機能の両方が含まれています。
